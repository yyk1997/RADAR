...
class ABL(BackdoorDefense):
    ...
    def train_isolation(self):
        ...
        for epoch in range(0, self.isolation_epochs):
            ...
            # val_atk(self.args, model_ascent)
            test(model_ascent, test_loader=self.test_loader, poison_test=True, poison_transform=self.poison_transform, num_classes=self.num_classes, source_classes=config.source_class, all_to_all=('all_to_all' in self.args.dataset))
