        ...
        if True:
            if epoch % 5 == 0:
                if args.dataset == 'imagenet':
                    ...
                    torch.save(model.state_dict(), supervisor.get_model_dir(args))
                else:
            ...
            if epoch == 15:
                save_dir = os.path.join(supervisor.get_poison_set_dir(args), f'full_base_aug_seed=2333_15epochs.pt')
...
if args.poison_type == 'none':
    if args.no_aug:
        torch.save(model.state_dict(), f'models/{args.dataset}_vanilla_no_aug.pt')
        torch.save(model.state_dict(), f'models/{args.dataset}_vanilla_no_aug_seed={args.seed}.pt')
    else:
        torch.save(model.state_dict(), f'models/{args.dataset}_vanilla_aug.pt')
        torch.save(model.state_dict(), f'models/{args.dataset}_vanilla_aug_seed={args.seed}.pt')