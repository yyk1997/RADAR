'''codes used to conduct RADAR on the GTSRB dataset
'''
import argparse
import os, sys
import time
...
def iterative_poison_distillation(inspection_set, suspicious_set ,clean_set, params, args, distillation_ratio, debug_packet=None, start_iter=0 ,momentums = params['momentums'],lambs = params['lambs'],lrs = params['lrs']):
    ...
    for confusion_iter in range(start_iter, num_confusion_iter):
        ...
        if confusion_iter < 2:  # lr=0.01 for round 0,1
            pretrain_epochs = 100
            pretrain_lr = 0.01
            distillation_iters = 500
        elif confusion_iter < 3:  # lr=0.01 for round 0,1
            pretrain_epochs = 100
            pretrain_lr = 0.01
            distillation_iters = 500
        elif confusion_iter < 4:  # lr=0.01 for round 0,1
            pretrain_epochs = 100
            pretrain_lr = 0.01
            distillation_iters = 2500
        # elif confusion_iter < 5:  # lr=0.01 for round 0,1
        #     pretrain_epochs = 40
        #     pretrain_lr = 0.01
        #     distillation_iters = 1000
        # else:  # lr=0.01 for round 0,1
        #     pretrain_epochs = 40
        #     pretrain_lr = 0.01
        #     distillation_iters = 1500
...
for epoch in range(1, epochs + 1):  # train backdoored base model
    ...
    if epoch >= 5 and epoch % 5 == 0:
            ...
            if index <= 1:
                if index < 1:
                    ...
                    first_one_second_percent = set(sorted_overall_clean_indices[
                                               math.ceil(3 * ten_percent_count):math.ceil(
                                                   5 * ten_percent_count)])
            else:
        ...
        if index == 2:
            ...
            distilled_samples_indices, median_sample_indices, confuse_model = iterative_poison_distillation(poisoned_set,suspicious_set,
        ...
        eta = len(suspicious_indices_set_list) / 26640
